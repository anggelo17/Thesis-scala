...
/** The `Graph` class stores vertex-labeled directed graphs using an adjacency
  ...
  */
class GraphCAR [TLabel: ClassTag] (val ch:      Array [SET [Int]],
  ...
  extends Cloneable
{
  ...
  /** Clone (make a deep copy) of this graph.
    */
 // override def clone: GraphCAR [TLabel] = new GraphCAR (ch.clone, label.clone, inverse)
  ...
  /** Determine whether 'this' graph and graph 'g' have the same vertices
    ...
    */
  def same (g: GraphCAR[TLabel]): Boolean =
  {
...
object MGraphO
{
  ...
  /** Build an `MGraph` from a `Graph`.
    ...
    */
  def apply [TLabel: ClassTag] (gr: GraphCAR [TLabel], eLab: Map [Pair, TLabel], name: String): GraphCAR [TLabel] =
  {
    new GraphCAR (gr.ch, gr.label, eLab, gr.inverse, name)
  } // apply
  ...
  /** Build an `MGraph` from a `Tree`.
    ...
    */
//  def apply (tree: Tree, name: String = "t", inverse: Boolean = false): GraphCAR [Double] =
//  {
//    GraphCAR (Graph (tree, name, inverse), tree.labelMap, name)
//  } // apply
...
object GraphCARO
{
  ...
  val g1p = new GraphCAR (g1.ch, g1.label, g1.elabel, true, g1.name)    // with parents
val q1p = new GraphCAR (q1.ch, q1.label, q1.elabel, true, q1.name)    // with parents
...
/** The `GraphTest` object is used to test the `Graph` class using the graphs
  ...
  */
object GraphTestCAR extends App
{
  import GraphCAR._
  g1.printG ()
  q1.printG ()
...
/** The `GraphTest2` object is used to test the `Graph` class using a randomly
  ...
  */
object GraphTestCAR2 extends App
{
