...
import scalation.graphalytics._

//import scala.collection.mutable.{Map, Set}
import scala.collection.immutable.{Set => SET}
import scalation.graphalytics.mutable.LabelFunctions.{gChildLabels, gParentLabels}
...
/** The `MDualIso` class provides an implementation for Subgraph Isomorphism
  ...
  */
class DualIsoCAR[TLabel: ClassTag](g: GraphCAR[TLabel], q: GraphCAR[TLabel])
  extends GraphMatcherCAR(g, q) {
  private val duals = new DualSim2CAR(g, q)
  ...
  override def bijections(): SET[Array[Int]] = {
    ...
    saltzDualIso(duals.DualSimCAR(phi), 0)
    ...
    if (depth == q.size) {
      ...
      for (i <- phi(depth)){
        ...
        if (!contains(phi, depth, i)) {
          ...
          // phi = phiCopy.map(x => x)
          saltzDualIso(duals.DualSimCAR(phiCopy), depth + 1) // solve recursively for the next depth
...
object MDualIsoCARTest1 extends App {
 ...
 // def dossomething(phiCpy: Array[mutable.Set[Int]]) = ???
...
/** The `MDualIsoTest3` object is used to test the `MDualIso` class.
  ...
  */
object MDualIsoCARTest3 extends App {
