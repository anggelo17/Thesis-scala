...
import scalation.graphalytics.GraphCAR
...
object LabelFunctionsCAR
{
  ...
  /** Given the query graph and value of query vertex it returns its corresponding
    ...
    */
  def qChildLabels [TLabel: ClassTag] (q: GraphCAR [TLabel], u: Int): MultiSet [TLabel] =
  {
    val cLabel = new MultiSet [TLabel] ()
    for (c <- q.ch(u)) { cLabel += q.label(c); println (s"q.label($c): " + q.label(c)) }
    cLabel
  } // qChildLabels
  ...
  /** Given the query graph and value of query vertex it returns its corresponding
    ...
    *  @param u  query graph vertex
    */
  def qParentLabels [TLabel: ClassTag] (q: GraphCAR [TLabel], u: Int): MultiSet [TLabel] =
  {
    val pLabel = new MultiSet [TLabel] ()
    println(s"u==$u")
    for (p <- q.pa(u)) {
      pLabel += q.label(p)
      println(s"q.label($p)= "+q.label(p))
    }
    pLabel
  } // qParentLabels
  ...
  /** Given the value of query vertex, graph vertex and phi, it returns the child
    ...
    */
  def gChildLabels [TLabel: ClassTag] (g: GraphCAR [TLabel], v: Int, u: Int, chu: SET [Int],
  ...
  /** Given the value of query vertex, graph vertex and phi, it returns the parent
    ...
    *  @param phi  array of mappings from a query vertex u to { data graph vertices v }
    */
  def gParentLabels [TLabel: ClassTag] (g: GraphCAR [TLabel], v: Int, u: Int, pau: SET [Int],
                                        phi: Array [SET [Int]]): MultiSet [TLabel] =
  {
    var st = SET [Int] ()
