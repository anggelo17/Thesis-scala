...
object GraphIO
{
     ...
     */
    def apply (name: String, base: String = BASE_DIR, ext: String = EXT, sep: Char = ','): Graph[TLabel] =
    {
        val gFile = base + name + ext                             // relative path-name for file
     ...
     */
    def read2Files (lFile: String, eFile: String, inverse: Boolean = false): Graph[TLabel] =
    {
     ...
     *  @param inverse   whether to store inverse adjacency sets (parents)
     */
    def read2PajekFile (lFile: String, eFile: String, inverse: Boolean = false): Graph[TLabel] =
    {
        val lLines = fromFile (lFile).getLines          // get the lines from lFile
